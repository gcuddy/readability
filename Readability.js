...
Readability.prototype = {
  ...
  REGEXPS: {
    ...
    unlikelyCandidates:
      /-ad-|ai2html|banner|breadcrumbs|combx|comment|community|cover-wrap|disqus|extra|footer|gdpr|header|legends|menu|related|remark|replies|rss|shoutbox|sidebar|skyscraper|social|sponsor|supplemental|save-story|saveStory|ad-break|agegate|pagination|pager|popup|yom-remote/i,
      ...
      if (
        ...
        try {
          ...
          if (parsed.datePublished) {
            metadata.date = parsed.datePublished;
          }
          if (parsed.image) {
            if (typeof parsed.image === "string") {
              metadata.image = parsed.image;
            } else if (
              Array.isArray(parsed.image) &&
              parsed.image[0] &&
              typeof parsed.image[0].url === "string"
            ) {
              metadata.image = parsed.image[0].url;
            }
          }
          return;
        } catch (err) {
  ...
  _getArticleMetadata: function (jsonld) {
    ...
    metadata.image = this._unescapeHtmlEntities(metadata.image);
    metadata.date = this._unescapeHtmlEntities(metadata.date);
